...
def authenticate():
    ...
    # time.
    if os.path.exists('secrets/token/token.pickle'):
        with open('secrets/token/token.pickle', 'rb') as token:
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                'secrets/credentials/credentials.json', SCOPES)
            creds = flow.run_local_server(port=0)
        # Save the credentials for the next run
        with open('secrets/token/token.pickle', 'wb') as token:
